...
Long-context sequence modeling forces an uncomfortable tradeoff:

1. We want strong, content-dependent reasoning over long windows.
2. We want training and inference to stay cheap enough to run on a single, real GPU budget.

Transformers are excellent at content-dependent routing, but full attention scales quadratically with sequence length. So memory and latency can become the limiting factor long before model quality does.
...
In this tutorial, treat Mamba as three layers at once:
...
Classic RNNs are linear-time, but their hidden-state dynamics are often hard to optimize over long horizons. Gradients either vanish, explode, or force heavy gating machinery that still struggles with very long dependencies.

Modern SSM-based models improve this by giving the recurrence a more principled dynamical structure. You can think of it as replacing a "generic recurrent matrix" with a better-behaved family of update rules. S4 made that practical for deep sequence modeling, and Mamba adds token-dependent selectivity so the memory policy can change from one token to the next.
...
We'll use the following symbols throughout:
...
In the block implementation, an easy way to avoid confusion is to read dimensions in this order:
...
We'll often highlight critical objects with color:
