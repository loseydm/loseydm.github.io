...
Those topics are best treated separately, once the core recurrence is fully internalized.
...
$$
x_t = \bar{A}*t \odot x*{t-1} + \left(\Delta_t \odot B_t\right) \odot u_t,
$$
...
$$
x_t = \bar{A}*t \odot x*{t-1} + (\Delta_t \odot B_t) \odot u_t,
\qquad
...
$$
x' = \mathrm{SiLU}(\mathrm{DWConv}_{\text{causal}}(x)).
$$
...
$$
y = \mathrm{SelectiveScan}(u = x', \Delta, A, B, C, D_{\text{skip}}).
$$
...
$$
\Delta_t = \mathrm{softplus}(\Delta^{\mathrm{raw}}*t + b*\Delta).
$$
...
\bar{A}^t x_0 + \sum_{k=0}^{t-1} \bar{A}^k \bar{B} u_{t-k}.
$$
...
x_t
\sim
\bar{A}*t \odot \bar{A}*{t-1} \odot \cdots \odot \bar{A}_{s+1}
\odot
...
$$
x_t = \bar{A}*t x*{t-1} + \bar{B}_t u_t.
$$
...
\left(\prod_{j=1}^{t} \bar{A}*j\right) x_0
+
\sum*{s=1}^{t}
\left(
...
$$
\mathrm{conv_state} \in \mathbb{R}^{B \times D_{\mathrm{inner}} \times (K - 1)}.
$$
...
Once that selective recurrence is internalized, the rest of Mamba becomes far less mysterious. The architecture is no longer a collection of disconnected implementation details. It is a coherent answer to one modeling problem: how to build a recurrent memory that is both structured and content-aware.
